...
def test_feedback_lambda_success(setup_table, dynamodb_client, table_name):
    ...
    assert 'Sent 1 feedback requests.' in json.loads(response['body'])


def test_feedback_lambda_no_events_yesterday(setup_table, dynamodb_client, table_name):
    from app import lambda_handler
    ...
    response = lambda_handler({}, None)
    assert response['statusCode'] == 200
    assert 'Sent 0 feedback requests.' in json.loads(response['body'])
